...
public class Configuration
{
	...
	// 否开启自动驼峰命名规则(camel case)映射，即从经典数据库列名 A_COLUMN到经典 Java属性名 aColumn的类似映射。
	protected boolean mapUnderscoreToCamelCase;
	// 当开启时，任何方法的调用都会加载该对象的所有属性。
	protected boolean aggressiveLazyLoading;
	// 是否允许单一语句返回多结果集
	protected boolean multipleResultSetsEnabled = true;
	// 使用自增主键,允许 JDBC 支持自动生成主键
	...
	// 当没有为参数提供特定的 JDBC类型时，为空值指定JDBC类型。
	...
	public KeyGenerator getKeyGenerator(String id) {
		return (KeyGenerator)keyGenerators.get(id);
	}
	...
	public ResultMap getResultMap(String id) {
		return (ResultMap)resultMaps.get(id);
	}
	...
	public ParameterMap getParameterMap(String id) {
		return (ParameterMap)parameterMaps.get(id);
	}
	...
	public MappedStatement getMappedStatement(String id, boolean validateIncompleteStatements) {
		...
		return (MappedStatement)mappedStatements.get(id);
	}
		...
		if ((!rm.hasNestedResultMaps()) && (rm.getDiscriminator() != null)) {
			for (Map.Entry<String, String> entry : rm.getDiscriminator().getDiscriminatorMap().entrySet()) {
				...
				if (hasResultMap(discriminatedResultMapName)) {
					ResultMap discriminatedResultMap = (ResultMap)resultMaps.get(discriminatedResultMapName);
