...
class PooledConnection implements InvocationHandler {
  ...
  // 记录当前PooledConnection对象所在的PooledDataSource对象.该对象PooledConnection是从该
  // PooledDataSource中获取的，当调用close方法时会将PooledConnection放回该PooledDataSource中
  private final PooledDataSource dataSource; // 在PooledDataSource
  ...
  private int connectionTypeCode; // 由数据库URL,用户名和密码计算出来的hash值
  
  // 检测当前PooledConnection是否有效，主要是为了防止程序通过close方法将连接归还给连接池之后，依然通过
  // 该连接操作数据库
  ...
  public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
    ...
    // 如果调用close方法，则将其重新放入连接池，而不是真正的关闭数据库连接
