...
public class MapperMethod {
  ...
  private <K, V> Map<K, V> executeForMap(SqlSession sqlSession, Object[] args) {
    Map<K, V> result;
    // 对输入参数进行预处理:{param1:args1,param2:args2}
    Object param = method.convertArgsToSqlCommandParam(args);
    // 判断是否存在RowBound类型参数
    if (method.hasRowBounds()) {
      RowBounds rowBounds = method.extractRowBounds(args);
  ...
  public static class SqlCommand {
    ...
    public SqlCommand(Configuration configuration, Class<?> mapperInterface, Method method) {
      ...
      if (ms == null) {
        if (method.getAnnotation(Flush.class) != null) { // 是否为@Flush注解
          name = null;
          type = SqlCommandType.FLUSH;
        } else {
          // 抛出BindingException异常
      ...
      System.out.println("本次sql执行类型SqlType = " + type + " neam = " + name);
    }
    ...
    private MappedStatement resolveMappedStatement(Class<?> mapperInterface, String methodName,
      ...
      } else if (mapperInterface.equals(declaringClass)) { // mapperInterface与declaringClass相等
        return null;
      }
  ...
  public static class MethodSignature {
    ...
    private Integer getUniqueParamIndex(Method method, Class<?> paramType) {
      ...
      for (int i = 0; i < argTypes.length; i++) { // 遍历MethodSignature对象方法的参数列表
        // 判断该参数是否为(RowBounds类型|ResultHandler类型)
