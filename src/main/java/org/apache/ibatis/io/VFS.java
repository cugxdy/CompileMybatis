...
public abstract class VFS {
  ...
  /*
   * 记录了用户自定义的VFS实现类。VFS.addImplClass()方法会将指定的VFS实现对应的Class对象添加到
   * USER_IMPLEMENTATIONS集合中
   */
  ...
  private static class VFSHolder {
    ...
    static VFS createVFS() {
      ...
      // 遍历impls集合，依次实例化VFS对象并检测VFS对象并检测VFS对象是否有效，一旦得到有效的VFS对象，则结束循环
      VFS vfs = null;
      for (int i = 0; vfs == null || !vfs.isValid(); i++) {
        ...
        try {
          ...
          if (vfs == null || !vfs.isValid()) {  // VFS.isValid()方法是一个抽象方法
