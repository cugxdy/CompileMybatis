...
/*	  <cache
    ...
    if (context != null) {
      ...
      // 获取<cache>节点的type属性，默认值为PerpetualCache实现类
      ...
      // 获取<cache>节点的flushInterval属性，默认值为null(刷新间隔时间)
      Long flushInterval = context.getLongAttribute("flushInterval");
      
      // 获取<cache>节点的size属性，默认值为null
      Integer size = context.getIntAttribute("size");
      
      // 获取<cache>节点的readOnly属性，默认值为false(只读缓存)
  ...
  private ResultMap resultMapElement(XNode resultMapNode, List<ResultMapping> additionalResultMappings) throws Exception {
    ...
    for (XNode resultChild : resultChildren) { 
      if ("constructor".equals(resultChild.getName())) {
    	// 处理<constructor>节点,生成resultMapping对象并添加到resultMappings集合中
...
/*	<discriminator javaType="int" column="gender">
    ...
    for (XNode caseChild : context.getChildren()) {
      ...
      discriminatorMap.put(value, resultMap); // 存储在Map对象中
    }
  ...
  private ResultMapping buildResultMappingFromContext(XNode context, Class<?> resultType, List<ResultFlag> flags) throws Exception {
    ...
    // 解析javaType、typeHandler、jdbcType并生成相应的Class类型
    Class<?> javaTypeClass = resolveClass(javaType);
    @SuppressWarnings("unchecked")
    Class<? extends TypeHandler<?>> typeHandlerClass = (Class<? extends TypeHandler<?>>) resolveClass(typeHandler);
    JdbcType jdbcTypeEnum = resolveJdbcType(jdbcType);
