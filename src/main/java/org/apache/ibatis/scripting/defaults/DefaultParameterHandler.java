...
public class DefaultParameterHandler implements ParameterHandler {
  ...
  // 语句创建，BoundSql中也记录了对应参数的名称和相关属性
  ...
  public void setParameters(PreparedStatement ps) {
    ...
    if (parameterMappings != null) {
        System.out.println("实参类型  = " + parameterObject.getClass().getName());
      for (int i = 0; i < parameterMappings.size(); i++) {
        ParameterMapping parameterMapping = parameterMappings.get(i);
        System.out.println("绑定第" + i + "实参:  " + parameterMapping.toString());
        ...
        if (parameterMapping.getMode() != ParameterMode.OUT) {
          ...
          try {
        	// 通过typeHandler.setParameter()方法会调用parameterMapping.set*()方法为SQL语句绑定相应的实参
        	System.out.println("jdbcType = " + jdbcType);  
        	System.out.println("TypeHandler = " + typeHandler.getClass().getName());  
